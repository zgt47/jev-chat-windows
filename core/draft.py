...
SYSTEM = (
    ...
    "也不能凭空声称「报告在我手机里」「我手上有截图」「我这边有文件」「我可以截给你/拍给你/把记录发你」。"
    "知识库里即使写了“检测结果正常/维保记录完整”，也只代表这个事实本身，不代表你当前持有可发送的报告、截图或附件。"
    "对方要求这些现实动作时，不要承诺执行；只回应当前确实能确认的文字事实，必要时留给人工处理。\n"
...
_FAKE_ACTION = re.compile(
    r"(?:"
    r"(?:我|这边).{0,10}(?:发你|给你发|传你|给你传|发过去|传过去"
    r"|查过|查好了|查到了|核对过|核对好了|看过报告|看了报告|打开了|看到了附件"
    r"|打过电话|电话打了|转账了|付款了)"
    r"|(?:截图|报告|检测报告|维保记录|出险记录|照片|图片|视频|文件|附件).{0,10}"
    r"(?:发你|给你|传你|截给你|拍给你|给你看|发过去|传过去)"
    r"|(?:截|截图|拍).{0,6}(?:给你|发你|传你|几页给你)"
    r"|(?:报告|截图|照片|图片|视频|文件|附件).{0,8}(?:在我手机|在我电脑|在我手里|我这有|我这里有)"
    r"|(?:我手机|我电脑|我手里|我这边).{0,8}(?:有|存着|放着).{0,8}"
    r"(?:报告|截图|照片|图片|视频|文件|附件)"
    r")",
    re.I,
)
...
def draft_candidates(messages: list, relationship: str, provider: str = "deepseek",
    ...
    user += (
        ...
        "已经查过外部记录、已经打电话或已经完成其它现实动作；也禁止声称这些资料就在你手机/电脑/手里，"
        "或承诺截图、拍照、传文件给对方。知识库事实不等于你持有可发送的附件。"
